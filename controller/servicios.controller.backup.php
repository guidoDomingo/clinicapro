...
class ControladorServicios {
     ...
     */
    static public function ctrObtenerDiasDisponibles($doctorId, $servicioId = 0, $fechaEspecifica = null) {
        try {
            ...
            $diasDisponibles = ModelServicios::mdlObtenerDiasDisponibles($doctorId, $servicioId, $fechaEspecifica);
            
            // Si se especifica una fecha, filtrar solo esa fecha
            if (!empty($fechaEspecifica)) {
                ...
                if (strpos($fechaEspecifica, '/') !== false) {
                    ...
                    error_log("ctrObtenerDiasDisponibles: Formato de fecha no reconocido: $fechaEspecifica", 3, '/var/log/clinica/servicios.log');
                }
                
                if ($fechaFiltro) {
                    $diasDisponibles = array_filter($diasDisponibles, function($dia) use ($fechaFiltro) {
                        return $dia['fecha'] == $fechaFiltro;
                    });
                    
                    error_log("ctrObtenerDiasDisponibles: Filtrado por fecha específica $fechaEspecifica -> $fechaFiltro, encontrados " . count($diasDisponibles) . " días", 3, '/var/log/clinica/servicios.log');
                }
            } else {
