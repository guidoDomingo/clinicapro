<?php
/**
 * Script para verificar los permisos de especialidades
 */
...
echo "<h1>Verificación de Permisos para Especialidades</h1>";
...
// Verificar el permiso de administrar_especialidades
try {
    $stmt = $conexion->prepare("SELECT permiso_id, nombre, descripcion FROM permisos WHERE nombre = 'administrar_especialidades'");
    $stmt->execute();
    $permiso = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($permiso) {
        echo "<p style='color:green'>✓ El permiso 'administrar_especialidades' existe con ID: {$permiso['permiso_id']}</p>";
    } else {
        echo "<p style='color:red'>✗ El permiso 'administrar_especialidades' no existe.</p>";
...
if (!isset($permiso) || count($roles) === 0) {
    ...
    if (isset($_POST['accion']) && $_POST['accion'] === 'crear_permiso') {
        try {
            ...
            if (!isset($permiso)) {
                $stmt = $conexion->prepare("INSERT INTO permisos (nombre, descripcion) VALUES ('administrar_especialidades', 'Permiso para gestionar las especialidades médicas')");
                $stmt->execute();
                echo "<p style='color:green'>✓ Permiso creado correctamente.</p>";
                
                // Obtener el ID del permiso recién creado
                $stmt = $conexion->prepare("SELECT permiso_id FROM permisos WHERE nombre = 'administrar_especialidades'");
                $stmt->execute();
                $permiso = $stmt->fetch(PDO::FETCH_ASSOC);
            }
...
echo "<p>Verificación completada. <a href='index.php?ruta=especialidades'>Ir al módulo de Especialidades</a></p>";
?>
